...
// LockSurface.qml, the same LockAuth.qml, the same theme data, the same
// `df-lock` PAM service, so a correct password really is checked and a wrong
// one really is rejected -- but nothing holds the compositor. Ctrl-C ends it,
// Escape on an empty field gives the keyboard back, and a successful password
...
ShellRoot {
    ...
    // The lock shares one of these between every screen's surface; one window
    // needs only one, but it is the same component and the same conversation.
    LockAuth {
        id: lockAuth
