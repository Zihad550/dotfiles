...
test("Bluetooth Page groups nearby devices and keeps volatile BlueZ objects out of rows", () => {
    ...
    assert.match(bluetoothPage, /text:\s*"CONNECTED"[\s\S]*model:\s*root\.groups\.connected/);
    assert.match(bluetoothPage, /text:\s*"PAIRED"[\s\S]*model:\s*root\.groups\.paired/);
    assert.match(bluetoothPage, /text:\s*"AVAILABLE"[\s\S]*root\.groups\.available/);
    assert.match(bluetoothPage, /"Scanning for devices…"/);
    ...
    assert.match(bluetoothPage, /forgetAvailable:\s*Model\.hasSecondaryActions\(/);
    assert.match(bluetoothPage, /Qt\.RightButton\)\s*root\.secondaryAction\(/);
    assert.match(bluetoothPage, /if \(!Model\.hasSecondaryActions\(row\)\) return;/);
    assert.match(bluetoothPage, /text:\s*"Forget"/);
